...
# Usage:
# ./initialize_users_h200-hp.sh [--dry-run] [path/to/h200-hp-users.csv]

set -euo pipefail

shopt -s extglob

CSV_FILE="h200-hp-users.csv"
CSV_ARG_SEEN=false
DRY_RUN=false

usage() {
    echo "Usage: $0 [--dry-run] [path/to/h200-hp-users.csv]"
}
...
set_assoc_limits() {
    ...
    if [[ "$account_name" == *_h200_r ]]; then
        run_sacctmgr modify user name="$user_name" account="$account_name" set GrpTRES=gres/gpu=2 MaxWall=1-00:00:00 MaxSubmitJobs=20
...
while IFS= read -r raw_line || [[ -n "$raw_line" ]]; do
    ...
    user_name="${raw_line%%,*}"
    accounts_field="${raw_line#*,}"

    user_name="$(trim_field "$user_name")"
    accounts_field="$(trim_field "$accounts_field")"

    if [[ "$accounts_field" == \"*\" && "$accounts_field" == *\" ]]; then
        accounts_field="${accounts_field:1:${#accounts_field}-2}"
    fi

    if [[ -z "$user_name" || -z "$accounts_field" ]]; then
        echo "Skipping malformed row $line_number in $CSV_FILE"
        continue
    fi

    IFS=',' read -r -a accounts <<< "$accounts_field"
    for account_name in "${accounts[@]}"; do
        account_name="$(trim_field "$account_name")"
