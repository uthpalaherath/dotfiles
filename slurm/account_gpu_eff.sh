#!/usr/bin/env bash
# account_gpu_eff_summary.sh
# Compute GPU-hour-weighted Avg GPUEff with summary info
# Usage: ./account_gpu_eff_summary.sh -p PARTITION -a ACCOUNT -s STARTDATE -e ENDDATE
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -e|--end)   END="$2";   shift 2;;
    -h|--help)
      echo "Usage: $0 [-p PARTITION] [-a ACCOUNT] [-s STARTDATE] [-e ENDDATE]"
      echo "Example: $0 -p h200alloc -a duke -s 2025-10-01 -e 2025-10-31"
      exit 0;;
...
CMD=(slurm-report -r "$PART" -A "$ACCOUNT" -S "$START" --summary --plain)
if [[ -n "$END" ]]; then
  CMD+=(-E "$END")
fi
...
if [[ -z "$REPORT" ]]; then
  echo "No summary output for partition=$PART account=$ACCOUNT between $START and ${END:-now}"
  exit 0
fi

# If the report says "No valid jobs found", stop
if printf '%s\n' "$REPORT" | grep -qi "No valid jobs found"; then
  echo "No valid jobs found for partition=$PART account=$ACCOUNT between $START and ${END:-now}"
  exit 0
fi

awk -v part="$PART" -v account="$ACCOUNT" -v start="$START" -v end="$END" '
  BEGIN {
    FS="[[:space:]]+";
    sum_weighted = 0;
    sum_hours = 0;
