...
#
# Usage:
#   partition_gpu_eff.sh -r PARTITION -S STARTDATE -E ENDDATE -A ACCOUNT
#
# Example:
#   partition_gpu_eff.sh -r h200alloc -S 2025-12-01 -E 2025-12-09
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "Example: $0 -p h200alloc -s 2025-12-01 -e 2025-12-09 -A acct1,acct2 --telegraf"
      exit 0;;
...
printf '%s\n' "$STATS" | awk -v part="$PART" -v start="$START" -v end="$END" -v acct="$ACCOUNT" -v csv="${CSV_FORMAT}" -v telegraf="${TELEGRAF_FORMAT}" -v ts="$TS_NANOS" '
  BEGIN {
    if (csv == "1") {
      OFS = ",";
    } else {
  ...
  END {
    ...
    if (telegraf == "1") {
      ...
      if (acct != "") {
        tagstr = tagstr ",account=" acct;
      }

      printf "slurm_gpu_partition_efficiency,%s users=%di,total_gpu_hours=%.4f,avg_gpu_eff=%.4f,avg_gpu_mem_eff=%.4f %s\n",
             tagstr, nusers, total_gpuhours, timew_avg, avg_mem_eff, ts;
      exit;
    }
