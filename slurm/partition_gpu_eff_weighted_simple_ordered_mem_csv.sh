...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [-p PARTITION] [-s STARTDATE] [-e ENDDATE] [-c]"
      echo "Example: $0 -p h200alloc -s 2025-12-01 -e 2025-12-09"
      echo "  -c, --csv    Output in CSV format for easy copy-paste"
      exit 0;;
...
printf '%s\n' "$STATS" | awk -v part="$PART" -v start="$START" -v end="$END" -v csv="'${CSV_FORMAT}'" '
  BEGIN {
    if (csv == "1") {
      OFS = ",";
    } else {
